 ...
 *
 * The drawer is portalled to <body>. The header it is launched from has
 * `backdrop-filter`, and an element with a backdrop filter becomes the
 * containing block for its fixed-position descendants — so rendered in place,
 * `fixed inset-0` meant "the 72px header", not the viewport. On phones the
 * panel collapsed to the header's height: the links had no room at all and the
 * buttons spilled out over the hero.
 */
