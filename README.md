...
| The internal quotation process         | `/admin/rfqs` — each desk (customer service, supply chain, procurement, shipping) takes its own hand-off; an order estimated at $100,000 or more is referred to Mabani for a technical study first; prices, studies and files are recorded per desk. A quotation can only be sent once it has been approved. See the workflow section below |
...
```
customer service  →  supply chain  →  ┌ procurement ─(>= $100k)→ Mabani (study) ┐
                                      │                                                  │→ supply chain
                                      └ shipping ────────────────────────────────────────┘   compiles
